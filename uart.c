...
int main(){
	...
	int ID[ID_LENTH];
	int i;
	struct timeval tpstart[3],tpend[3];
	unsigned long diff; 
	wiringPiSetup();
	pinMode(0,OUTPUT);
	pinMode(2,INPUT);
	...
	while(1) {
		if(digitalRead(2) == 1) {
			...
			if(idAuth(ID) == -1){
				...
				printf("%s\n",User[1].returnFlag);
				delay(4000);
				if(strcmp(User[idAuth(ID)].returnFlag,"0") == 0) {//User[idAuth(ID)].returnFlag == "0"){
					gettimeofday(&tpstart[idAuth(ID)],NULL);
					system("clear");
					printf("Take Your Umbrella!,%s\n",User[idAuth(ID)].name);
					User[idAuth(ID)].returnFlag = "1";
					if(write_db(idAuth(ID),"1")==0) printf("Update is done\n");
					...
					if(write_db(idAuth(ID),"0")==0) printf("Update is done\n");
					delay(3000);system("clear");
                                        digitalWrite(0,HIGH);
                                        delay(4000);system("clear");
                                        digitalWrite(0,LOW);				
